...
class NewTrainer(Trainer):
    def __init__(self, **kwargs):
        super(NewTrainer, self).__init__(**kwargs)
        ## load ft model
    ...
    def compute_loss(self, model, inputs, return_outputs = False):
        ...
        ## ys to get correct logit
        labels = torch.tensor(inputs["labels"])
        y_onehot = torch.zeros(outputs.logits.shape)
        ...
        xs = torch.tensor(inputs["input_ids"])
        lens = (xs != 0).sum(1)
        _ = self.ftmodel(xs, lens)
        fl = self.ftmodel.logits
        bl = outputs.logits
        # print("fl", fl.shape)
        # print("bl", bl.shape)

        ## compute denoising loss
        loss = self.lsm(bl).mul(y_onehot).sum(-1)
        regularizer = torch.log(torch.softmax(fl,-1).mul(torch.softmax(bl,-1)).sum(-1))
        loss = torch.neg(loss.sub(regularizer).sum())
